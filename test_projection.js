...
async function buildWithSetblockByRegion(schematicData, startPos) {
  ...
  // 生成并执行/give指令，给予自己前128个方块所需的全部材料
  console.log('正在获取建造前128个方块所需材料...');
  for (const [blockName, count] of Object.entries(initialMaterialCount)) {
    const giveCommand = `/give @s ${blockName} ${count}`;
    console.log(`执行命令: ${giveCommand}`);
  ...
  for (let rz = 0; rz < regionsZ; rz++) {
    for (let rx = 0; rx < regionsX; rx++) {
      ...
      if (startZ < length) {
        ...
        // 生成并执行/give指令，给予自己区域所需的全部材料
        console.log('正在获取建造所需材料...');
        for (const [blockName, count] of Object.entries(materialCount)) {
          const giveCommand = `/give @s ${blockName} ${count}`;
          console.log(`执行命令: ${giveCommand}`);
          bot.chat(giveCommand);
